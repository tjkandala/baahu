  ...
  describe(`${diffType} list diffing`, () => {
    ...
    Object.keys(listMap).map(testCase => {
      test(testCase, () => {
        ...
        const { mount, emit } = baahu<ListEvent>();

        const ListMach = createListDiffComponent(listOne, listTwo, diffType);

        $root = mount(ListMach, $root) as HTMLElement;
